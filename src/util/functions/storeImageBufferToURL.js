...
const storeImageBufferToURL = async (id, buffer, updateDBFunc, isThumbnail, height, width, nsfw) => {
  ...
  if (response) {
    const rows = await updateDBFunc(id, cdnURL, buffer, height, width, nsfw, buffer.length, fileExtension);
